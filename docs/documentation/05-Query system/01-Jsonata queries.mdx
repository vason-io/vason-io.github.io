...
click the left query button or use <Key>Alt</Key> + <Key>Q</Key>.

For JSON analysis from the right panel, click the right query button or use <Key>AltRight</Key> + <Key>Q</Key>.
...
Click on the run button or use <Key>Ctrl</Key> + <Key>Enter</Key> to execute it and witness the result.
